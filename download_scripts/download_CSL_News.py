...
def download_files(output_directory, download_pose):
    ...
    for i in range(1, 2):
        ...
        # Usa curl al posto di wget
        command = ["curl", "-L", url, "-o", file_path]

        """
        # Usa 7z per estrarre senza cartelle superiori (-o indica cartella output)
        unzip_command = ["7z", "x", "-y", file_path, f"-o{video_folder}"]
        """

        ###############
        unzip_command = ["C:\\Program Files\\7-Zip\\7z.exe", "x", "-y", file_path, f"-o{video_folder}"]
        ################
        ...
        try:
            ...
            error_message = f"file {url} download or unzip failed: {e}\n"
            print(error_message)
    ...
    for data_url in data_urls:
        ...
        command = ["curl", "-L", data_url, "-o", data_path]
        subprocess.run(command, check=True)

    # delete zip files (opzionale)
    # delete_command = f"rmdir /s /q \"{RGB_zip_folder}\""
